...
def _comprimir_imagen_b64(b64: str, calidad: int = 60) -> str:
    """
    Recomprime una imagen JPEG base64 con menor calidad para reducir su peso.
    Úsado cuando una página individual es rechazada por ser demasiado grande.
    """
    ...
    try:
        ...
        return b64   # si falla la compresión, devolver la original
...
def _llamar_claude_lab_vision(
    ...
    Estrategia de manejo de payload grande (BadRequestError 400):
      1. Lote > 1 imagen  → dividir a la mitad y reintentar cada sublote.
      2. Imagen sola       → recomprimir con calidad 50 (peso ~40% del original)
                             y reintentar UNA vez.
      3. Imagen sola ya recomprimida y aún rechazada → omitir con aviso.
    Esto garantiza que ninguna página se salte sin intentar reducir su tamaño.
    """
    if not imagenes_b64:
        return []
    ...
    try:
        ...
        if len(imagenes_b64) > 1:
            # Lote grande → dividir a la mitad
            mitad = len(imagenes_b64) // 2
            st.caption(f"   ↳ Lote grande ({len(imagenes_b64)} págs), dividiendo en 2 sublotes de {mitad}…")
            ...
            # Página sola rechazada
            if not _reintento:
                # Primer intento fallido → recomprimir a calidad 50 y reintentar
                st.caption("   ↳ Página pesada, recomprimiendo a calidad 50%…")
                img_comprimida = _comprimir_imagen_b64(imagenes_b64[0], calidad=50)
                return _llamar_claude_lab_vision(
                    client, [img_comprimida], _reintento=True
                )
            else:
                # Ya se recomprimió y sigue rechazada → omitir con aviso claro
                st.warning(
                    "⚠️ Una página no pudo procesarse ni siquiera con compresión máxima. "
                    "Puede ser una imagen de muy alta resolución. Se omite y se continúa."
                )
                return []
...
def render_herramienta_lab(client: anthropic.Anthropic) -> None:
    ...
    # ── Subida y extracción ─────────────────────────────────────────────────
    # FIX: el file_uploader y el botón son INDEPENDIENTES del display.
    # El historial se carga desde BD siempre, no depende de que el PDF esté cargado.
    ...
    # Mostrar mensaje de éxito de la extracción anterior si existe
    _lab_msg = st.session_state.pop("_lab_ultimo_resultado", None)
    if _lab_msg:
        if _lab_msg.get("ok"):
            st.success(_lab_msg["texto"])
        else:
            st.warning(_lab_msg["texto"])

    if uploaded_pdf:
        if st.button("🔍 Iniciar extracción cruzada", type="primary", key="btn_lab"):
            pdf_bytes = uploaded_pdf.read()
            with st.spinner("Analizando PDF con Claude…"):
                muestras = analizar_reporte_laboratorio(client, pdf_bytes)

            if not muestras:
                # FIX: No hacer return — mostrar error Y seguir mostrando historial
                st.error(
                    "❌ No se encontraron muestras. "
                    "Verifica que el PDF contenga tablas analíticas de laboratorio."
                )
            else:
                st.info(f"✅ {len(muestras)} muestra(s) identificadas. Guardando…")
                errores = 0
                for m in muestras:
                    try:
                ...
                # FIX: guardar mensaje en session_state ANTES del rerun
                # para que aparezca en el siguiente ciclo sin perder el estado
                n_ok = len(muestras) - errores
                if errores:
                    st.session_state["_lab_ultimo_resultado"] = {
                        "ok": False,
                        "texto": f"Guardadas {n_ok} muestras. {errores} con errores.",
                    }
                else:
                    st.session_state["_lab_ultimo_resultado"] = {
                        "ok": True,
                        "texto": f"✅ {n_ok} muestras archivadas permanentemente en la base de datos.",
                    }
                ...
                try:
                    registrar_evento(
                        st.session_state.proyecto_actual,
                        "LABORATORIO",
                        f"{n_ok} muestra(s) extraídas del PDF de laboratorio",
                        st.session_state.get("usuario_actual", "Ingeniero"),
                ...
                # FIX: rerun solo al guardar exitosamente — limpia el uploader
                # y recarga el historial actualizado desde BD
                st.rerun()
