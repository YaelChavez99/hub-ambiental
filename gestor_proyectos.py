    ...
    try:
        ...
        c.execute("""
            SELECT
                ...
                SUM(CASE WHEN dl.rebase_nom THEN 1 ELSE 0 END) AS n_rebase
            FROM proyectos p
        ...
        return [
            {
                "id":            r[0],  "nombre":     r[1],
                "uso_suelo":     r[2],  "estado":     r[3] or "ACTIVO",
...
def render_gestor_proyectos(usuario_actual: str = "Ingeniero") -> None:
    ...
    if not proyectos:
        st.info("No hay proyectos registrados. Crea uno en la barra lateral.")
        return
